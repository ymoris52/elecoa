...
// ページの基本情報をセット
$PAGE->set_url('/blocks/elecoa_grades/view.php', array('courseid' => $courseid));
$PAGE->set_title(get_grade_string('gradelist', false));
$PAGE->set_heading($course->fullname);
$PAGE->navbar->add(get_grade_string('gradelist', false));
...
if (!$ismobile) {
    echo '<div class="elecoa-grade-csv-link">',
         '<a href="viewcsv.php?', $_SERVER['QUERY_STRING'], '">', get_grade_string('downloadascsv'), '</a>',
         '</div>';
}

// 検索条件表示 (PC)
if (!$ismobile) {
    show_search_criteria($courseid, $users, $items, $conditions);
}
...
if ($ismobile) {
    ...
    echo '<script type="text/javascript">YUI().use("event", "node", function(Y) { Y.on("domready", function() { Y.all(".elecoa-grade-conditions input.submit-button").hide();Y.on("change", function() { Y.one(this).ancestor("form").submit(); }, ".elecoa-grade-conditions .update-on-change");}); });</script>';
}
...
function show_search_criteria($courseid, $users, $items, $conditions) {
    ...
    if ($ismobile) {
         // モバイル用更新ボタン
    echo '  <input class="elecoa-mobile-btn" type="submit" value="', get_grade_string('update'), '">';
    } else {
         // PC用更新ボタン
    echo '  <input class="submit-button" type="submit" value="', get_grade_string('update'), '">';
    }
    echo '</form>',
         '</div>';
}
...
function show_selector_users($users, $conditions, $param) {
    ...
    if (!$ismobile) {
        ...
        foreach ($users as $user) {
            echo '<div class="elecoa-mobile-checkbox">';
...
function show_selector_items($items, $conditions, $param) {
    ...
    if (!$ismobile) {
        ...
        foreach ($items as $item) {
            echo '<div class="elecoa-mobile-checkbox">';
            echo '<input class="update-on-change" type="checkbox"  name="', $param, '" id="elecoa-grade-input-item', htmlspecialchars($item->elecoaid), '" value="', htmlspecialchars($item->elecoaid), '"', in_array($item->elecoaid, $conditions['itemid']) ? ' checked="checked"' : '', '><label for="elecoa-grade-input-item', htmlspecialchars($item->elecoaid), '">', htmlspecialchars($item->title), '</label>';
            echo '</div>';
        }
    }
}
...
function show_view_row($id, $user, $section, $name, $completion, $success, $score, $lessontime, $lessonperiod, $totalperiod, $details, $type) {
    ...
    if ($ismobile) {
        if ($type == 'header') {
            echo '<li data-role="list-divider" class="elecoa-mobile-li elecoa-mobile-list-divider elecoa-mobile-bar-b">', get_grade_string('gradelist'), '</li>';
        }
        if ($type == 'row') {
            echo '<li class="elecoa-mobile-btn elecoa-mobile-btn-up-d elecoa-mobile-li">';
            echo $details;
        ...
        // ID
        cell($id, 'narrow center');
        // ユーザー
        cell($user, 'narrow center');
        // セクション
        cell($section, 'center');
        // 教材
        cell($name, 'wide center');
        // 完了状態
        cell($completion, 'narrow center');
        // 合否
        cell($success, 'narrow center');
        // 得点
        cell($score, 'wide center');
        // 最終学習終了日時
...
function output_user_item_aggregation($user_item_records, $conditions) {
    ...
    if ($ismobile) {
        $details = '<a href="details.php?courseid=' . $course->id . '&amp;userid=' . $userid. '&amp;elecoaid=' . $elecoaid . '&amp;parentquery=' . urlencode($query_string) . '">';
    } else {
